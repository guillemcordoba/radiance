...
pub fn validate(op: Op) -> ExternResult<ValidateCallbackResult> {
    match op.to_type::<EntryTypes, LinkTypes>()? {
        OpType::StoreEntry(store_entry) => {
            match store_entry {
                OpEntry::CreateEntry { app_entry, action } => {
                    match app_entry {
                        EntryTypes::Deed(deed) => {
                            validate_create_deed(
            ...
            match update_entry {
                OpUpdate::Entry {
                    ...
                    match (app_entry, original_app_entry) {
                        (EntryTypes::Deed(deed), EntryTypes::Deed(original_deed)) => {
                            validate_update_deed(
                                action,
                                deed,
                                original_action,
                                original_deed,
            ...
            match delete_entry {
                OpDelete::Entry { original_action, original_app_entry, action } => {
                    match original_app_entry {
                        EntryTypes::Deed(deed) => {
                            validate_delete_deed(action, original_action, deed)
                        }
                    }
                }
                _ => Ok(ValidateCallbackResult::Valid),
        ...
        OpType::RegisterCreateLink {
            ...
            match link_type {
                LinkTypes::AllDeeds => {
                    validate_create_link_all_deeds(
                        action,
                        base_address,
                        target_address,
                        tag,
        ...
        OpType::RegisterDeleteLink {
            ...
            match link_type {
                LinkTypes::AllDeeds => {
                    validate_delete_link_all_deeds(
                        action,
                        original_action,
                        base_address,
                        target_address,
        ...
        OpType::StoreRecord(store_record) => {
            match store_record {
                ...
                OpRecord::UpdateEntry {
                    ...
                    match app_entry {
                        EntryTypes::Deed(deed) => {
                            let result = validate_create_deed(
                                EntryCreationAction::Update(action.clone()),
                                deed.clone(),
                            )?;
                            if let ValidateCallbackResult::Valid = result {
                                let original_deed: Option<Deed> = original_record
                                    .entry()
                                    .to_app_option()
                                    .map_err(|e| wasm_error!(e))?;
                                let original_deed = match original_deed {
                                    Some(deed) => deed,
                                    None => {
                                        return Ok(
                                            ValidateCallbackResult::Invalid(
                ...
                OpRecord::DeleteEntry { original_action_hash, action, .. } => {
                    ...
                    match original_app_entry {
                        EntryTypes::Deed(original_deed) => {
                            validate_delete_deed(action, original_action, original_deed)
                        }
                    }
                }
                OpRecord::CreateLink {
